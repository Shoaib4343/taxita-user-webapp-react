...
import React, { useState, useEffect } from "react";
import { 
  FaTimes, FaTrash, FaEdit, FaSave, FaGasPump, FaOilCan, FaCar, FaShieldAlt, 
  FaTools, FaDotCircle, FaCarSide, FaMoneyCheckAlt, FaCogs, FaPhoneAlt, 
  FaIdBadge, FaWrench, FaBalanceScale, FaSoap, FaUserTie, FaHome, FaBox, 
  FaParking, FaFileInvoice 
} from "react-icons/fa";
import toast from "react-hot-toast";
import { getTransactionsApi, deleteTransactionApi, updateTransactionApi } from "../services/dashboard";
import { showDeleteConfirmation, showSuccessAlert, showErrorAlert } from "../components/SweetAlert2Confirmation";
...
const ExpenseTransactionsModal = ({ 
  open, 
  onClose, 
  selectedDay, 
  onTransactionDeleted,
  ...
  // Get expense accounts from context
  const { expenseAccounts, loading: accountsLoading } = useAccounts();
  ...
  const handleEditTransaction = (transaction) => {
    ...
    setEditForm({
      account_id: transaction.account_id || '',
      amount: transaction.debit || transaction.amount || '',
      transaction_type: 'debit',
      transaction_date: transaction.transaction_date || '',
      description: transaction.description || '',
      attachment: null
  ...
  const getExpenseAccount = (accountId) => {
    return expenseAccounts.find(account => account.id === parseInt(accountId));
  };

  const getExpenseTypeDisplay = (accountId, accountType) => {
    const account = getExpenseAccount(accountId);
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            {loading ? (
                    ...
                    {editingTransaction === transaction.id ? (
                      ...
                      <div className="grid grid-cols-12 gap-3 px-6 py-4 bg-blue-50/50 border-l-4 border-blue-500">
                        ...
                        <div className="col-span-3 flex items-center">
                          <CustomDropdown
                            ...
                            value={editForm.account_id}
                            onChange={(e) => handleFormChange('account_id', e.target.value)}
                            options={expenseAccounts.map(expense => expense.id)}
                            placeholder="Select Expense Type"
                            displayFormatter={(id) => {
                              const expense = expenseAccounts.find(exp => exp.id === id);
                              return expense ? expense.title : 'Unknown';
                            }}
                            className="text-sm z-[60]"
                            containerClassName="z-[60]"
                      ...
                      <div className="grid grid-cols-12 gap-3 px-6 py-4 text-sm">
                        ...
                        <div className="col-span-3 flex items-center text-gray-900">
                          <div className="flex items-center gap-2">
                            {(() => {
                              const account = getExpenseAccount(transaction.account_id);
                              if (account) {
                              ...
                              return <span className="truncate">{getExpenseTypeDisplay(transaction.account_id, transaction.account_type)}</span>;
                            })()}
                          </div>
                        </div>
                        <div className="col-span-2 flex items-center text-gray-900 font-semibold">
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-gray-500">
                ...
                <p className="text-gray-500">No expense transactions have been added for this date.</p>
              </div>
            )}
          </div>
        </div>
