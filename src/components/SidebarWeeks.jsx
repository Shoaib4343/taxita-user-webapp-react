...
const SidebarWeeks = ({ weeksData, onSelect }) => {
  const [selectedWeekId, setSelectedWeekId] = useState(weeksData[0].id);
  const [selectedDayId, setSelectedDayId] = useState(weeksData[0].days[0].id);
  const [dropdownOpen, setDropdownOpen] = useState(false);
  ...
  // when selecting week from dropdown → reset day to first day
  const handleWeekSelect = (week) => {
    setSelectedWeekId(week.id);
    setSelectedDayId(week.days[0].id);
    setDropdownOpen(false);
    // send both week and day to parent
    onSelect && onSelect({ week, day: week.days[0] });
  };

  // when selecting day inside the week
  const handleDaySelect = (day) => {
    setSelectedDayId(day.id);
    onSelect && onSelect({ week: selectedWeek, day });
  };
    ...
    <aside className="w-80 bg-white border-r border-gray-200 flex flex-col">
      ...
      <div className="p-4 border-b border-gray-200 sticky top-0 bg-white z-20">
        ...
        <div className="relative">
          ...
          >
            <span className="truncate font-medium">
              Week {weeksData.findIndex((w) => w.id === selectedWeekId) + 1} w/c:{" "}
              {formatDate(selectedWeek.days[0].id)} &nbsp; £0
            </span>
            <FaChevronDown
