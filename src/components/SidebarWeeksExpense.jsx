...
const SidebarWeeksExpense = ({ onSelect, refreshTrigger, activeTradingYear, apiRefreshTrigger }) => {
  const [weeks, setWeeks] = useState([]);
  const [days, setDays] = useState([]);
  const [selectedWeek, setSelectedWeek] = useState(null);
  const [selectedDay, setSelectedDay] = useState(null);
  ...
  const fetchWeeks = useCallback(async () => {
    try {
      ...
      console.error("Error fetching expense weeks:", err);
      return [];
    }
  }, []);
  ...
  const fetchDays = useCallback(async (weekNumber) => {
    try {
      ...
      console.error("Error fetching expense days:", err);
      return [];
    } finally {
      setLoadingDays(false);
    }
  ...
  const initializeComponent = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error initializing expense component:", error);
    } finally {
      setLoading(false);
    }
  }, [fetchWeeks, findWeekContainingToday, todayDate, fetchDays, onSelect]);
  ...
  }, [selectedWeek?.week_number, selectedDay?.date, isRefreshing, weeks, days]);
  ...
  // Listen for trading year changes - Same pattern as Income
  useEffect(() => {
    if (apiRefreshTrigger > 0) {
      console.log('SidebarWeeksExpense: Refreshing data due to trading year change');
      const timeoutId = setTimeout(() => {
        refreshData();
      }, 100);
  ...
  return (
    <aside className="w-80 bg-white border-r border-gray-200 flex flex-col">
      ...
      <div className="p-4 border-b border-gray-200 sticky top-0 bg-white z-20">
        <h3 className="text-sm font-semibold mb-3 flex items-center gap-2 text-gray-800">
          <IoCalendarOutline className="text-red-500" /> Select Your Week
        </h3>

        <div className="relative">
          <button
            onClick={() => setDropdownOpen(!dropdownOpen)}
            className={`w-full border border-gray-300 rounded-lg px-3 py-3 text-left text-gray-700 hover:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-300 flex justify-between items-center text-sm disabled:opacity-50 disabled:cursor-not-allowed ${
              isRefreshing ? "opacity-75 cursor-wait" : ""
            }`}
            disabled={loadingDays || isRefreshing}
            aria-label="Select week"
            ...
            >
              {weeks.map((week) => (
                <li key={week.week_number} role="option">
                  <button
                    onClick={() => handleWeekSelect(week)}
                    className={`w-full flex justify-between items-center px-3 py-3 hover:bg-red-100 transition-colors duration-150 text-left ${
                      selectedWeek?.week_number === week.week_number
